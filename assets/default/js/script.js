...
$(function() {
	...
	$('.abahsoft_app_menu ul li').click(function() {
		...
		$('#sebaran_kecamatan').hide();
		$('#sebaran_kelurahan').hide();
		$('.markcol').hide();
...
function showkelurahan(){
	var kd_kec = $('#csebaran_kecamatan').val(); 
		$.ajax({
		type: "POST",
		url: base_url+'map/list_kelurahan',
		data: {kd_kec:kd_kec},
		dataType: 'html',
			success: function(data) {
			  document.getElementById('csebaran_kelurahan').innerHTML = data;
		},
	});
}
...
function showQuickSearch(id) {
	...
	var id = id;
		if(gol=='01'){
			$('#sebaran_kecamatan').show();
			$('#sebaran_kelurahan').show();
			$('.markcol').show();
		}
	var kec = $('#csebaran_kecamatan').val();
	var kel = $('#csebaran_kelurahan').val();
	...
	//if(kw!='') {
		$.ajax({
			...
			//data  : 'qs='+kw,
			data : {qs:kw,kd_skpd:kd_skpd,gol:gol,id:id,kec:kec,kel:kel},
			cache : false,
			success: function(datas) {
				var data = $.parseJSON(datas);
				...
				if(data.status) {
					...
					for(var i=0; i<data.responden.length; i++) {
						...
						if( (data.responden[i].lat !='') && (data.responden[i].lat !=null) && (data.responden[i].lat !='0') ){ 
						  var kdbarang = "'"+data.responden[i]['kd_brg']+"'";
						   if(gol=='01'){
							   if(data.responden[i].status_fasilitas=='1'){
							  	var catikon = 'markers/7.png';
							   }else{
								   if(data.responden[i].status_sertifikat=='1'){
									var catikon = 'markers/9.png';
								   }else if(data.responden[i].status_sertifikat=='2'){
									var catikon = 'markers/10.png';
								   }else if(data.responden[i].status_sertifikat=='3'){
									var catikon = 'markers/26.png';
								   }else{
									var catikon = 'markers/8.png';
								   }
							   }
						   }else{
							  var catikon = 'markers/'+data.responden[i]['kd_brg'].split(".").join("").substr(0,6)+'.png';
						   }
						  /* if(data.responden[i].kd_riwayat=='1' || data.responden[i].kd_riwayat=='3' || data.responden[i].kd_riwayat=='4' || data.responden[i].kd_riwayat=='10'){
							  	var catikon = 'markers/010107.png';
						  }else if(data.responden[i].kd_riwayat=='2' || data.responden[i].kd_riwayat=='5' || data.responden[i].kd_riwayat=='6' || data.responden[i].kd_riwayat=='7'){
							  	var catikon = 'markers/010104.png';
						  }else{
								//asli var catikon = 'markers/'+data.responden[i]['kd_brg'].split(".").join("").substr(0,6)+'.png';
						 } */
						 ...
						 //asli   $('#abahsoft_quick_search_result').append('<div class="abahsoft_quick_search_list"><i class="icon-target-2" style="font-size:8px;cursor:pointer;" onClick="showPencarianAsetMap('+idbarang+')"></i> <span style="cursor: pointer;" onClick="showAsetDetail('+idbarang+','+kdbarang+');">'+data.responden[i].no+' <br> '+data.responden[i].tgl_reg+' | '+data.responden[i].nilai+'<br>'+data.responden[i].kd_brg+' | '+data.responden[i].detail_brg+'</span></div>');
							$('#abahsoft_quick_search_result').append('<div class="abahsoft_quick_search_list"><i class="icon-target-2" style="font-size:14px;cursor:pointer;color:blue" onClick="showPencarianAsetMap('+idbarang+')">&nbsp</i> <span style="cursor: pointer;" onClick="showPencarianAsetMap('+idbarang+');">'+data.responden[i].no+' <br> '+data.responden[i].tgl_reg+' | '+data.responden[i].nilai+'<br>'+data.responden[i].kd_brg+' | '+data.responden[i].detail_brg+'</span></div>');
						   // $('#abahsoft_quick_search_result').append('<div class="abahsoft_quick_search_list"><i class="icon-target-2" style="font-size:14px;cursor:pointer;color:blue" onClick="pointerdata('+idbarang+')"></i> <span style="cursor: pointer;" onClick="pointerdata('+idbarang+');">'+data.responden[i].no+' <br> '+data.responden[i].tgl_reg+' | '+data.responden[i].nilai+'<br>'+data.responden[i].kd_brg+' | '+data.responden[i].detail_brg+'</span></div>');
	...
	var id = id;
		$.ajax({
			...
			success: function(datas) {
				...
				$('#abahsoft_quick_search').animate({'width':'230px'}, 500);
				/* setTimeout(function() { 
					$('#abahsoft_close_quick_search').fadeIn(); 
				}, 600);
				$('.abahsoft_quick_search_list').hide(); */
				for(var i=0; i<pid.length; i++) {
					markers[pid[i]].setMap(null);
			...
			success: function(datas) {
				...
				if(data.status) {
					...
					for(var i=0; i<data.responden.length; i++) {
						...
						$('#abahsoft_add_search_result').append('<div class="abahsoft_add_search_list"><i class="icon-target-2" style="font-size:14px;cursor:pointer;color:blue" onClick="addRespondenMap('+noregteks+','+idbarang+','+kdbarang+','+detailbrg+','+alamat+','+nilai+')">&nbsp</i> <span style="cursor: pointer;" onClick="showAsetDetail('+idbarang+','+kdbarang+');">'+data.responden[i].no+' <br> '+data.responden[i].tgl_reg+' | '+data.responden[i].nilai+'<br>'+data.responden[i].kd_brg+' | '+data.responden[i].detail_brg+'</span></div>');
						//end of ikram tambah
